...
基于 v2.5.6 的核心改动：
- 对已匹配 XGB 的股票，不再把旧的 sector_data_missing / yuanjun_data_missing 当作主因
- 诊断更聚焦真实阻塞原因：risk、price、hot、weekly、breakout
"""
...
def collect_flags(item: Dict[str, Any]) -> List[str]:
    ...
    # 如果 XGB 已匹配，就过滤旧缺失标签
    if item.get("xgb_pool_matched"):
        out = [x for x in out if not any(k in x for k in STALE_MISSING_KEYWORDS)]

    return list(dict.fromkeys(out))


def diagnose_one_candidate_v259(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 只有未匹配 XGB 时，才把板块/援军缺失作为诊断原因
    if not item.get("xgb_pool_matched"):
        if _has(flags, "sector_data_missing") or _has(flags, "xgb_pool_no_match"):
            reasons.append("板块/股池未匹配")
            if category == "unknown":
...
def diagnose_tail_candidates_v259(items: List[Dict[str, Any]]) -> Dict[str, Any]:
    diagnostics = [diagnose_one_candidate_v259(x) for x in items]
    ...
    xgb_matched = [x for x in items if x.get("xgb_pool_matched")]
    ...
    return {
        ...
        "xgb_matched_count": len(xgb_matched),
        "top_near_candidates": top_near,
        "diagnostics": diagnostics,
    }


def format_tail_diagnosis_report_v259(items: List[Dict[str, Any]]) -> str:
    d = diagnose_tail_candidates_v259(items)
    lines: List[str] = []
    lines.append("【尾盘候选诊断 v2.5.9】")
    lines.append("")
    lines.append(f"候选总数：{d['total']}")
    lines.append(f"行情新鲜：{d['fresh_ok_count']}")
    lines.append(f"XGB匹配：{d['xgb_matched_count']}")
    ...
    for x in d["top_near_candidates"][:10]:
        lines.append(
            ...
            f"theme={x.get('theme_name') or '-'} pools={x.get('xgb_pools') or '-'} | "
            f"{','.join(x['reasons'])}"
        )
    return "\n".join(lines)
