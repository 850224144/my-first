...
from .tail_confirm_report_v262 import (
    build_tail_confirm_summary_v262,
    format_tail_confirm_summary_md_v262,
)

try:
    from .parquet_safe_writer_v263 import write_parquet_safe_v263
except Exception:
    write_parquet_safe_v263 = None
...
def _write_parquet(rows: List[Dict[str, Any]], path: str | Path) -> None:
    """
    v2.6.3:
    使用安全 parquet writer，避免 pyarrow list/string 混列。
    """
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)

    cleaned = [clean_record_fields(x) for x in rows]

    if write_parquet_safe_v263 is not None:
        write_parquet_safe_v263(cleaned, p)
        return
...
def run_tail_confirm_from_tail_focus_v262(
    *,
    trade_date: Optional[str] = None,
    tail_focus_path: str | Path = "data/watchlist_tail_focus.parquet",
    output_results_path: str | Path = "data/tail_confirm_results.parquet",
    ...
    for c in cleaned_rows:
        ...
        # 保留 observe gate 字段，避免 process_tail_candidate 输出覆盖
        for k in [
            "observe_quality",
            "observe_priority",
            "risk_bucket",
    ...
    _write_parquet(results, output_results_path)

    summary = build_tail_confirm_summary_v262(
        trade_date=trade_date,
        results=results,
    )

    report_dir = Path(report_dir)
    json_path = report_dir / f"tail_confirm_summary_{trade_date}.json"
    md_path = report_dir / f"tail_confirm_summary_{trade_date}.md"

    _write_text(json_dumps_safe(summary), json_path)
    _write_text(format_tail_confirm_summary_md_v262(summary, results), md_path)
