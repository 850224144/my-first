...
from .tail_reason_compactor_v265 import add_tail_explanations_v265
from .tail_confirm_report_v265 import (
    build_tail_confirm_summary_v265,
    format_tail_confirm_summary_md_v265,
    build_tail_daily_section_v265,
)
...
def run_tail_confirm_from_tail_focus_v265(
    *,
    trade_date: Optional[str] = None,
    tail_focus_path: str | Path = "data/watchlist_tail_focus.parquet",
    output_results_path: str | Path = "data/tail_confirm_results_v265.parquet",
    ...
    results = add_tail_explanations_v265(results, max_items=5)
    write_parquet_safe_v263(results, output_results_path)

    summary = build_tail_confirm_summary_v265(
        trade_date=trade_date,
        results=results,
        source_tail_focus_path=str(tail_focus_path),
    )

    report_dir = Path(report_dir)
    json_path = report_dir / f"tail_confirm_summary_v265_{trade_date}.json"
    md_path = report_dir / f"tail_confirm_summary_v265_{trade_date}.md"
    daily_path = report_dir / f"tail_daily_section_v265_{trade_date}.md"

    _write_text(json_dumps_safe(summary), json_path)
    _write_text(format_tail_confirm_summary_md_v265(summary, results), md_path)
    _write_text(build_tail_daily_section_v265(summary, results), daily_path)
