"""
v2.5.4 watchlist pipeline。

相比 v2.5.3：
- 默认从 DuckDB realtime_quote 读取 quote_map
- 默认补 current_price / fresh_quote
- 修正缺少板块数据时的援军硬拒绝
"""
...
from .realtime_quote_loader_v254 import load_realtime_quote_map_from_duckdb, quote_map_report
from .score_enricher_v254 import enrich_candidate_scores_v254
...
def build_realtime_quote_map_for_watchlist_v254(
...
def build_candidates_from_watchlist_v254(
    rows: List[Dict[str, Any]],
    *,
    ...
    for row in rows:
        ...
        # build_candidate 里会 normalize symbol；这里补一次 normalized key
        if quote is None:
            try:
...
def enrich_candidates_from_project_data_v254(
    ...
    *,
    ...
    for i, c in enumerate(candidates):
        ...
        enriched = enrich_candidate_scores_v254(
            c,
...
def preview_final_signals_from_watchlist_v254(
    *,
    ...
    auto_quote_map = quote_map
    if auto_quote_map is None:
        auto_quote_map = build_realtime_quote_map_for_watchlist_v254(
            rows,
    ...
    quote_report = quote_map_report(auto_quote_map, symbols)

    candidates = build_candidates_from_watchlist_v254(rows, quote_map=auto_quote_map)
    enriched = enrich_candidates_from_project_data_v254(
        candidates,
        duckdb_path=duckdb_path,
        core_pools=core_pools,
        strategy_config=strategy_config,
