...
def build_tail_confirm_summary_v265(
    *,
    trade_date: str,
    results: List[Dict[str, Any]],
    source_tail_focus_path: str | None = None,
) -> Dict[str, Any]:
    enriched = add_tail_explanations_v265(results, max_items=5)
    ...
    return {
        ...
        "source_tail_focus_path": source_tail_focus_path,
        "total": len(enriched),
...
def format_tail_confirm_summary_md_v265(summary: Dict[str, Any], results: List[Dict[str, Any]]) -> str:
    rows = add_tail_explanations_v265(results, max_items=5)

    lines: List[str] = []
    lines.append(f"# Tail Confirm Summary v2.6.5 {summary.get('trade_date')}")
    lines.append("")
    lines.append("## 总览")
    lines.append("")
    ...
    if summary.get("source_tail_focus_path"):
        lines.append(f"- 来源文件：{summary.get('source_tail_focus_path')}")
    lines.append("")
    lines.append("## 信号分布")
    lines.append("")
...
def build_tail_daily_section_v265(summary: Dict[str, Any], results: List[Dict[str, Any]]) -> str:
    rows = add_tail_explanations_v265(results, max_items=4)
